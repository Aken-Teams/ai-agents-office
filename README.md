...
- **智慧需求釐清** - 需求不明確時，AI 主動提問並顯示可點選的選項按鈕（`[CHOICES]` 互動 UI）
- **5 種文件類型** - 支援 PPTX、DOCX、XLSX、PDF 生成 + HTML 互動簡報，各有專屬技能代理
- **即時串流** - 基於 SSE 的即時進度推送，可視化代理活動狀態
- **完成音效** - AI 任務完成時播放提示音（Web Audio API），方便背景多工
- **聊天嵌入圖表** - AI 回應中自動嵌入互動式圖表（Recharts: bar/line/area/pie/radar/scatter），支援放大、下載 PNG/SVG
- **互動式心智圖** - Markmap 驅動的可收合/展開樹狀心智圖，支援縮放、拖曳、下載
- **互動式地圖** - ECharts 世界地圖/區域地圖，支援 hover 資訊、地理數據視覺化
- **Mermaid 圖表** - 支援流程圖、甘特圖、ERD、序列圖等，自動偵測暗色/亮色主題
- **線上簡報** - HTML 互動式簡報，8 種主題風格（minimal/dark/gradient/neon/corporate/creative/elegant/tech），卡片式版面、Storyset 插圖、全頁浮水印
- **檔案內嵌預覽** - 聊天中直接預覽 PDF、Word、Excel、PPT 第一頁，含版本切換
- **檔案版本管理** - 自動快照備份，支援下載任意歷史版本
- **檔案上傳分析** - 上傳 CSV、Excel、PDF 等檔案，由 AI 代理分析並生成報告
- **分享功能** - 產生分享連結，讓未登入使用者也能檢視對話與檔案
- **CJK 字型支援** - PDF 生成自動偵測中日韓文字，切換 Noto Sans SC 字型
...
- **浮水印** - 生成文件自動加上浮水印（PPTX、DOCX、XLSX、PDF、HTML 簡報）
...
| 圖表/圖形 | Recharts, Mermaid, Markmap（互動式心智圖）, ECharts（互動式地圖） |
| 簡報引擎 | 自研 HTML 引擎（8 主題 + Reveal.js 相容導航） |
...
│   │   │   └── charts/         #   聊天嵌入圖表（ChatChart, ChatMermaid, ChatMindmap, ChatMap）
...
pnpm install
cp .env.example .env
# 編輯 .env，設定 JWT_SECRET、MySQL 連線資訊
pnpm run init-db   # 自動建立資料表 + 管理員帳號
```
...
| `CORS_ORIGINS` | `localhost` | CORS 允許來源（逗號分隔，部署時需設定） |
| `API_URL` | `http://localhost:PORT` | Next.js → Express API 代理目標 URL |
| `MYSQL_HOST` | `127.0.0.1` | MySQL 主機位址 |
| `MYSQL_PORT` | `3306` | MySQL 連接埠 |
| `MYSQL_DB` | `db_ai_agents` | MySQL 資料庫名稱 |
| `MYSQL_USER` | `root` | MySQL 使用者名稱 |
| `MYSQL_PASSWORD` | - | **必填**，MySQL 密碼 |
...
6. 複製 Client ID，設定到根目錄 `.env`：
   ```bash
   GOOGLE_CLIENT_ID=xxx.apps.googleusercontent.com
   ```
   > 前端會自動從根 `.env` 讀取，無需額外設定 `client/.env.local`。
...
| 方法 | 端點 | 說明 |
|------|------|------|
| POST | `/api/share/:conversationId` | 建立分享連結 |
| GET | `/api/share/:token` | 取得分享內容（公開） |
| DELETE | `/api/share/:conversationId` | 撤銷分享 |

### 上傳

| 方法 | 端點 | 說明 |
|------|------|------|
| POST | `/api/uploads` | 上傳檔案（multipart） |
| GET | `/api/uploads` | 列出上傳檔案 |
| DELETE | `/api/uploads/:id` | 刪除上傳檔案 |

### 用量

| 方法 | 端點 | 說明 |
|------|------|------|
| GET | `/api/usage` | 取得個人 Token 用量統計 |
...
| `slides-gen` | 工作者 | `.html` | 互動式 HTML 簡報（8 主題 + 圖表 + 地圖 + 心智圖 + 浮水印） |
...
| 互動地圖 | ` ```map ` | ECharts | Hover 資訊、地理數據視覺化、下載 |
...
MySQL 資料庫（`db_ai_agents`），執行 `pnpm run init-db` 會自動建立所有資料表：

- **`users`** - 使用者帳號，含角色（`user`/`admin`）、狀態（`active`/`suspended`/`pending`）、OAuth 資訊
- **`conversations`** - 對話工作階段，追蹤模式（`direct`/`orchestrated`）
- **`messages`** - 對話訊息（user/assistant/system）
- **`generated_files`** - 檔案註冊表（含版本控制）
...
sudo mysql -e "CREATE DATABASE db_ai_agents CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
sudo mysql -e "CREATE USER 'aiagents'@'localhost' IDENTIFIED BY 'your-password';"
sudo mysql -e "GRANT ALL PRIVILEGES ON db_ai_agents.* TO 'aiagents'@'localhost';"
...
cd ai-agents-office
cp .env.example .env
# 編輯 .env：設定 JWT_SECRET、MySQL 連線資訊、CORS_ORIGINS（如使用域名）

# 5. 一鍵安裝
pnpm run setup    # install + init-db + build
...
```powershell
# 1. 安裝 Node.js 18+、pnpm、MySQL 8.0+
# 2. 建立 MySQL 資料庫 db_ai_agents
# 3. 安裝 Claude CLI
npm install -g @anthropic-ai/claude-code
