...
- **封面**：用 word_insert_cover，不要自己用 insert_text 組。
  自己組出來會是幾行大字浮在一整頁空白裡，看起來像沒做完。
  版面照文件性質選：規格書報告用 report（會附一張文件資訊表）、
  彙整表點檢表用 form、報價單對外文件用 letterhead。
...
- **流程與架構要畫圖**：文件裡出現「流程」「架構」「步驟」「時程」就用 word_insert_diagram
  畫 mermaid 圖。用文字描述一個流程，讀者要自己在腦中畫一次。
...
只用 word_insert_text 寫字，一份文件就只會有「標題、章節、內文」三級——
做出來就是大文字配小文字，看起來像草稿。

真正的商務文件有更多層：欄位標籤跟它的內容不同色、附註比內文小、章節底下的前言
比內文大但比標題輕。這些用 **word_insert_block** 做，字級由區塊種類決定，你不用挑，
挑了反而會不一致。

- **左右並排的東西 → word_insert_layout**。報價單的「客戶資料 | 報價資訊」、
  信頭的「公司名靠左、地址靠右」、文末的簽核欄，都是並排而不是堆疊。
  只會往下寫的話，版面永遠不會像範本。
- 成對的資訊（客戶名稱、報價日期、文件版本）→ kind=fields，多的話用 kind=kv
- 說明、但書、注意事項 → kind=note
- 章節標題底下的一句話 → kind=lead
