...
export const EXCEL_TOOLS: ExcelToolSpec[] = [
  ...
  },
  {
    ...
    description:
      ...
      + '\n\n【可以用的東西】ctx（Excel.RequestContext）、workbook（= ctx.workbook）、'
      ...
      + '\n\n【Office.js 的規矩】先 .load("欄位") 再 await ctx.sync() 才讀得到值；'
      + '不要在迴圈裡 sync，一次 load 一整塊再純 JS 運算。範例：'
      + '\nconst sh = workbook.worksheets.getItem("工作表1");'
      + '\nconst rng = sh.getUsedRange(); rng.load("values"); await ctx.sync();'
