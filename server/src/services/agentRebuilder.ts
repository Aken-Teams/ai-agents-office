...
export async function agentRebuildSlide(
  ...
  const message = [
    ...
    `4. 如果是第 1 頁（封面頁），設計要有創意（色塊、幾何元素）`,
    `5. 如果是內容頁，保持專業風格，搭配 accent bar、footer 等元素`,
    '6. 如果資料有指定顏色欄位（backgroundColor、accentColor 等），**務必使用這些顏色作為整頁配色**',
    '7. 輸出檔名為 slide-output.pptx（僅 1 頁）',
    '8. 不需要產生 slides.json',
    '',
    '請開始作業。',
  ].join('\n');
