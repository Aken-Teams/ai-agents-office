...
/* ============================================================
   Slide Thumbnails — convert each slide to a PNG image
   ============================================================ */

/**
 * Convert a PPTX file to per-slide PNG images using LibreOffice.
 * Returns an array of base64-encoded PNG images (one per slide).
 * Falls back to an empty array if LibreOffice is unavailable.
 */
export async function generateSlideThumbnails(filePath: string): Promise<string[]> {
  const soffice = await findLibreOffice();
  if (!soffice) return [];

  const tmpDir = path.join(path.dirname(filePath), '.slide-thumbs');
  if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir, { recursive: true });

  const basename = path.basename(filePath, path.extname(filePath));
  const thumbDir = path.join(tmpDir, basename);

  // Check cache: if thumb dir exists and is newer than source, use it
  if (fs.existsSync(thumbDir)) {
    const srcStat = fs.statSync(filePath);
    const dirStat = fs.statSync(thumbDir);
    if (dirStat.mtimeMs > srcStat.mtimeMs) {
      return readThumbnailsFromDir(thumbDir);
    }
  }
  ...
  try {
    // LibreOffice: convert to PNG (one per slide)
    await execFileAsync(soffice, [
      '--headless',
      '--convert-to', 'png',
      '--outdir', thumbDir,
    ...
    // LibreOffice outputs a single PNG for single-page or numbered PNGs
    // For multi-slide: try using PDF → individual PNGs approach
    const pngs = fs.readdirSync(thumbDir).filter(f => f.endsWith('.png')).sort();

    if (pngs.length === 1) {
      // LibreOffice --convert-to png only gives first slide; use PDF-based approach
      // Convert to PDF first, then we'll return just the PDF preview approach
      // For now return the single thumbnail for all slides
      const singleThumb = fs.readFileSync(path.join(thumbDir, pngs[0]));
      return [singleThumb.toString('base64')];
    }

    return pngs.map(f => fs.readFileSync(path.join(thumbDir, f)).toString('base64'));
  } catch (err) {
    console.error('[FilePreview] Thumbnail generation failed:', err);
    return [];
  }
}

function readThumbnailsFromDir(dir: string): string[] {
  const pngs = fs.readdirSync(dir).filter(f => f.endsWith('.png')).sort();
  return pngs.map(f => fs.readFileSync(path.join(dir, f)).toString('base64'));
}

export { OFFICE_EXTENSIONS, findLibreOffice };
