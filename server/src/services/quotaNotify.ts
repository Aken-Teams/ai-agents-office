...
export async function notifyQuotaRequest(params: {
  ...
  currentCost: number;
  adminUrl?: string;
}): Promise<{ sent: boolean; recipients: number; detail?: string }> {
  ...
  const subject = `【額度申請】${params.requesterName} 提出額度調整申請`;
  const reviewLink = params.adminUrl ? `${params.adminUrl.replace(/\/$/, '')}/admin/quota-requests` : '';
  const body = `
    <div style="font-family:'Microsoft JhengHei',Arial,sans-serif;font-size:14px;color:#222;line-height:1.7;">
