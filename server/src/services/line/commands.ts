...
async function buildTeamFromTopic(lineUser: LineUserRow, topic: string): Promise<void> {
  ...
  const verdict = await moderateTeamTopic(topic, '無法建立這個團隊', { userId: lineUser.internal_user_id });
  if (!verdict.allowed) {
...
export async function handleSchedule(lineUser: LineUserRow, args: string): Promise<void> {
  ...
  const verdict = await moderateTeamTopic(question, '無法建立這個排程', { userId: lineUser.internal_user_id });
  if (!verdict.allowed) {
