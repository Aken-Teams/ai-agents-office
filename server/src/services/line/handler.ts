...
import { formatHitsForPrompt, countIndexedChunks } from '../personalRag.js';
...
import { getEmbedQueue, defaultJobOptions } from '../queue.js';
...
/**
 * Image / file events — download via the Content API, security-scan, save
 * to the user's _uploads dir, then enqueue an embed job. The actual text
 * extraction + RAG indexing happens in `workers/embeddingWorker.ts` so the
 * chat worker isn't blocked.
 *
...
async function handleIncomingFile(
  ...
  // Enqueue the heavy work. We persist a `user_uploads` row first so the
  // downstream tools can find the file by id if needed.
  ...
  await getEmbedQueue().add('index', {
    ...
    fileType: path.extname(originalName).slice(1).toLowerCase() || 'bin',
    storagePath: relPath,
...
async function runConversation(lineUser: LineUserRow, message: string): Promise<void> {
  ...
  // Personal RAG — only spend an embedding call when the user has indexed
  // docs. The orchestrator gets the formatted hits via `referenceContext`,
  // which becomes part of the router system prompt.
  let ragContext = '';
  try {
    const hasDocs = (await countIndexedChunks(userId)) > 0;
    if (hasDocs) {
      const hits = await hybridSearch({ userId, query: message, k: 4 });
      if (hits.length > 0) ragContext = formatHitsForPrompt(hits);
    }
  } catch (err) {
    console.warn('[LINE handler] RAG retrieval failed (non-fatal):', err);
  }
  ...
  const orchestrator = new Orchestrator(
    ...
    conversation.id,
    sseSink,
    [],            // uploadIds — Phase C will populate this
    'zh-TW',
    'assistant',   // category — LINE bots get cross-assistant context
    ragContext,    // referenceContext — personal RAG hits prepended to prompt
  );
