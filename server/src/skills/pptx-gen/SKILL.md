...
1. **每一個放內文 / bullet / 段落文字的 `addText` 都必須帶 `autoFit: true`** — 這樣文字太長時 pptxgenjs 會**自動縮字**塞進框內，而不是溢出撐爆到隔壁。這是防「溢出型跑版」最有效、成本最低的一招，**不可省略**（標題、頁碼等短字可免）。
   - **行距與段距（讓文字有呼吸空間、更好讀）**：bullet / 條列文字加上 `lineSpacingMultiple: 1.4`~`1.5` 和 `paraSpaceAfter: 8`~`10`（每個 bullet 之間留 8–10pt 間距）。這是純美觀加分、且因為有 `autoFit`，間距讓內容變高時會自動縮字，**不會跑版**。
...
3. **版型要「輪流變化」——從下面這組「安全版型」挑著用，同一份 deck 絕不要每頁都同一種**。每種版型的座標都已排好不重疊（圖表記得用吋），挑好直接套，不要自己另外亂算位置：
   ...
   - **H. 上卡 + 下左圖右文（三區塊，偶爾用來變化）**：上排 KPI 卡片用**下方「安全 KPI 卡片配方」照抄**（`cy=1.9`，卡片列到 `y≈45%`）；下方**左邊圖表** `{ x:0.7, y:3.7, w:5.6, h:2.6 }`（吋）、**右邊文字** `x:'52%', y:'49%', w:'44%', h:'37%'`。三區之間都留了間隙，**照抄座標、不要自己改**。
   > **鐵則：一份 deck 至少輪替用到 3–4 種不同版型，且不要連續兩頁用同一種。** 這是避免「每頁長一樣、看起來敷衍」的關鍵。
   > **以「兩區塊」版型（A~G）為主幹。H 這種三區塊版型只在「整份 deck 挑 1~2 頁」用來製造變化即可，不要每頁都用三區塊**——三區塊要同時擺對容易撞。要放更多內容就多開一頁。下筆放每個元素前，確認它的矩形不與已放上去的元素相交。
4. **KPI 數據卡片與圖表，絕不可放在同一個垂直（y）區間**——這是「數字壓在柱子上」的元兇。規則：
   - **絕不可把一張 stat 卡片「浮」在圖表上面**（例如在折線圖左下角疊一張「32.68%」卡片）。要強調某個數字，就把它**放進文字欄的 bullet**、或當圖表下方的一行說明，**不要**做成蓋在圖上的浮動卡片。
