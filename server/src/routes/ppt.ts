/**
 * /api/ppt — the PowerPoint add-in's half of the bridge.
 *
 * The third of the set (routes/excel.ts, routes/ppt.ts, this), section for
 * section. Everything structural is shared by design — auth, the SSE stream, the
 * run registry, attachments, conversation history — and living in three files
 * rather than one route with a dozen ternaries is deliberate: see
 * docs/multi-host.md. A shared route costs everyone who ever debugs one host.
 *
 * What is genuinely PowerPoint's and therefore here:
 *
 *   · CATEGORY — 'ppt-addin', so deck history never mixes with PowerPoint's or Excel's
 *   · the capability gate — much wider spread than PowerPoint's, because pictures need
 *     ImageCoercion (Office 2021) and the pane simply omits those tools without
 *     it. A person on Office 2019 is not behind on the ADD-IN and must not be
 *     told to restart PowerPoint; they are behind on Office, which is a
 *     different sentence and a different remedy.
 *   · the prompt — presentations are a layout problem, not a prose problem
 */
...
import {
  PPT_ASSISTANT_SYSTEM_PROMPT, DATA_SOURCE_PROMPT, LOCALE_PROMPT, resolveLocale,
} from '../services/pptContext.js';
import {
  registerRun, closeRun, callWorkbookTool, resolveToolCall, runBelongsTo,
} from '../services/excelBridge.js';
import { PPT_TOOL_NAMES, PPT_SERVER_TOOLS } from '../services/pptToolSpec.js';
...
/** Threads are filed under this, so deck history never mixes with the other two. */
const CATEGORY = 'ppt-addin';

/**
 * Model for the PowerPoint agent.
 *
 ...
 */
const PPT_MODEL = 'claude-opus-5';

const PPT_MODELS: { id: string; label: string; note: string; default?: boolean }[] = [
...
function resolveModel(requested?: string): string {
  const hit = PPT_MODELS.find(m => m.id === requested);
  return hit ? hit.id : PPT_MODEL;
}
 ...
 */
async function getOrCreatePptConversation(
  ...
  // or a host below PowerPointApi 1.3) — worse, because a rename then splits the
...
async function availableDataSources(
  ...
  const [mail, mailbox, onBehalf] = await Promise.all([
    ...
    kmEnabledFor('ppt') ? getKmOnBehalf(userId).catch(() => null) : Promise.resolve(null),
  ]);
...
router.post('/chat', async (req: Request, res: Response) => {
  ...
  const {
    ...
    // pane sends — SharedRuntime pins its JavaScript until PowerPoint is quit, so a
  ...
  const conversationId = await getOrCreatePptConversation(userId, docKey, docName, !!newConversation);
  ...
  // The ids go to the run so ppt_read_file can be answered here rather than
  // being sent down to PowerPoint and back for text this process already holds.
  ...
  // reopening the task pane does NOT reload its JavaScript — only quitting PowerPoint
  // does. Advertise only what THIS pane can execute, or the agent burns its turns
  // retrying 未知的工具.
  //
  // For PowerPoint this gate is doing more work than PowerPoint's. The capability
  // spread across Office builds is much wider: pictures and diagrams need
  // ImageCoercion 1.1, which means Office 2021, and on anything older the pane
  // leaves them out of its list entirely.
  //
  // So a short list here has two quite different causes, and they need two
  // different sentences. An old PANE is fixed by quitting PowerPoint. An old
  // OFFICE is not fixed by anything the person can do from the side bar, and
  // telling them to restart would send them round a loop that cannot end.
  const supported = Array.isArray(clientTools) && clientTools.length
    ...
    ? PPT_TOOL_NAMES.filter(n => clientTools.includes(n) || PPT_SERVER_TOOLS.includes(n))
    : PPT_TOOL_NAMES;
  const missing = PPT_TOOL_NAMES.filter(n => !supported.includes(n) && !PPT_SERVER_TOOLS.includes(n));
  const versionGated = new Set(['ppt_add_image', 'ppt_add_diagram']);
  ...
  if (staleClient.length) {
    write({
      ...
      data: `增益集版本較舊，少了 ${staleClient.length} 個功能（${staleClient.join('、')}）。`
        + '請完全關閉 PowerPoint 再重新開啟——只關掉側邊欄不會更新程式碼。',
    });
  } else if (missing.length) {
    write({
      type: 'info',
      data: '這個版本的 Office 不支援從增益集插入圖片（需要 Office 2021 以上），'
        + '所以這一輪不會有圖表或圖片。文字、圖形和排版都不受影響。',
    });
  }
  ...
  const parts: string[] = [];
  if (docName) parts.push(`（使用者目前開啟的簡報：${docName}）`);
  if (docContext) {
    parts.push(
      '以下是這份簡報的最新結構概覽，已經幫你讀好了，不需要再呼叫 ppt_get_overview：\n'
  ...
  if (uploaded.length) {
    parts.push(
      ...
      + `用 ppt_read_file 讀（index 從 1 到 ${uploaded.length}，長檔案用 part 一段一段拿）。`
      + '他多半是要你**依這些檔案產出一份簡報**——讀完就直接動手寫進文件，'
      + '不要只把內容複述一遍。',
    );
  ...
  // and ppt_read_attachment is what actually fetches one — same design as
  ...
  if (imageCount) {
    parts.push(
      `使用者在這則訊息附了 ${imageCount} 張圖片。用 ppt_read_attachment 看（index 從 1 到 ${imageCount}）。`
  ...
  const mcpKmOnBehalf = wanted.includes('km') && kmEnabledFor('ppt')
  ...
  if (wanted.length && !mounted.length) {
    write({ type: 'error', data: '你要求的外部資料來源目前都無法使用（可能是尚未連結 Outlook，或沒有 KM 權限）。這一輪只會用這份簡報的內容。' });
  ...
  const systemPrompt = (mounted.length
    ? PPT_ASSISTANT_SYSTEM_PROMPT + DATA_SOURCE_PROMPT(mounted)
    : PPT_ASSISTANT_SYSTEM_PROMPT) + localeBlock;
  const { emitter, abort } = spawnClaude(prompt, systemPrompt, {
    userId, conversationId,
    sandboxSubdir: '_agents/ppt-addin',
    ...
    model: chosenModel,
    mcpPptRunToken: bridgeToken,
    mcpPptTools: supported,
  ...
  // Task pane closed / PowerPoint quit / network dropped: kill the CLI and fail every
  ...
  emitter.on('event', (ev: SSEEvent) => {
    ...
    if (gap > 3000) {
      console.log(`[ppt] gap ${(gap / 1000).toFixed(1)}s → ${ev.type}`
...
// ─── Internal: ppt-mcp subprocess → bridge (loopback only) ───
...
export { internalRouter as pptInternalRoutes };
