...
import { getMailToken, getMailboxStatus, fetchFolders, fetchMessages, fetchMessageDetail, resolveCidImages, fetchAttachment } from '../services/outlookApi.js';
...
// mail call — say that plainly, and name the person who can actually fix it,
// instead of claiming the connection expired.
router.use(async (req: Request, res: Response, next) => {
  // /status answers for itself — it reports the reason rather than erroring.
  if (req.path === '/status') { next(); return; }
  const status = await getMailboxStatus(req.user!.userId);
  if (status.available) { next(); return; }
  res.status(403).json({ code: status.code, error: status.message });
});
...
router.get('/status', async (req: Request, res: Response) => {
  ...
  res.json({
    connected: !!token && status.available,
    mailAvailable: status.available,
    ...(token && !status.available ? { reason: status.code, message: status.message } : {}),
  });
});
