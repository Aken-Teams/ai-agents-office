...
router.post('/', async (req: Request, res: Response) => {
  const userId = req.user!.userId;
  const { title, skillId, category, instructions } = req.body;
  const id = uuidv4();
  ...
  // Custom assistant instructions (persona) — only meaningful for assistants.
  const agentInstructions = (effectiveCategory === 'assistant' && typeof instructions === 'string' && instructions.trim())
    ? instructions.trim().slice(0, 4000)
    : null;
  await dbRun(
    'INSERT INTO conversations (id, user_id, title, skill_id, mode, category, agent_instructions) VALUES (?, ?, ?, ?, ?, ?, ?)',
    id, userId, title || 'New Conversation', effectiveSkillId, mode, effectiveCategory, agentInstructions
  );
...
router.patch('/:id', async (req: Request, res: Response) => {
  const userId = req.user!.userId;
  const { title, status, instructions } = req.body;
  ...
  // Update assistant persona; '' clears it back to a general assistant.
  if (typeof instructions === 'string') {
    const val = instructions.trim() ? instructions.trim().slice(0, 4000) : null;
    await dbRun('UPDATE conversations SET agent_instructions = ? WHERE id = ?', val, conversation.id);
  }
