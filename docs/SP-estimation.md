...
本專案總計 **20 SP**（1 SP = NT$20,000），各階段分配如下：

```mermaid
pie title 專案 Story Point 分配 (共 20 SP)
    "需求確認 (2)" : 2
    "MVP 架構確認 (2)" : 2
    "開發中 (12)" : 12
    "驗收中 (4)" : 4
...
| 階段 | SP | 佔比 | 說明 |
|------|:--:|------|------|
| 需求確認 | 2 | 10% | 需求訪談、功能範圍確認、使用情境定義 |
| MVP 架構確認 | 2 | 10% | 技術架構設計、資料庫規劃、介面原型確認 |
| 開案確認 | 0 | 0% | — |
| 開發中 | 12 | 60% | 10 大模組開發、前後端整合、技能建置 |
| 驗收中 | 4 | 20% | 整合測試、安全驗證、效能調校、部署上線 |
| 已結案 | 0 | 0% | — |
| **合計** | **20** | **100%** | |
